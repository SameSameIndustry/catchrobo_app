...
type Light struct {
	Header std_msgs_msg.Header `yaml:"header"`// Optional header data
	Name string `yaml:"name"`// Light name
	Type uint8 `yaml:"type"`// Light type (from constant definitions)
	Pose geometry_msgs_msg.Pose `yaml:"pose"`// Light pose
	Diffuse std_msgs_msg.ColorRGBA `yaml:"diffuse"`// Light diffuse emission
	Specular std_msgs_msg.ColorRGBA `yaml:"specular"`// Light specular emission
	AttenuationConstant float32 `yaml:"attenuation_constant"`// Constant variable in attenuation formula
	AttenuationLinear float32 `yaml:"attenuation_linear"`// Linear variable in attenuation formula
	AttenuationQuadratic float32 `yaml:"attenuation_quadratic"`// Quadratic variable in attenuation formula
	Direction geometry_msgs_msg.Vector3 `yaml:"direction"`// Light direction
	Range float32 `yaml:"range"`// Light range
	CastShadows bool `yaml:"cast_shadows"`// Enable/disable shadow casting
	SpotInnerAngle float32 `yaml:"spot_inner_angle"`// Spotlight inner cone angle
	SpotOuterAngle float32 `yaml:"spot_outer_angle"`// Spotlight outer cone angle
	SpotFalloff float32 `yaml:"spot_falloff"`// Falloff between inner and outer cone
	Id uint32 `yaml:"id"`// Unique id of the light
	ParentId uint32 `yaml:"parent_id"`// Unique id of the light's parent
	Intensity float32 `yaml:"intensity"`// Light intensity
}
