...
type MaterialColor struct {
	...
	Entity Entity `yaml:"entity"`// Entity to change material color
	Ambient std_msgs_msg.ColorRGBA `yaml:"ambient"`// Ambient color
	Diffuse std_msgs_msg.ColorRGBA `yaml:"diffuse"`// Diffuse color
	Specular std_msgs_msg.ColorRGBA `yaml:"specular"`// Specular color
	Emissive std_msgs_msg.ColorRGBA `yaml:"emissive"`// Emissive color
	Shininess float64 `yaml:"shininess"`// Specular exponent
	EntityMatch uint8 `yaml:"entity_match"`// Entities that match to apply material color
}
