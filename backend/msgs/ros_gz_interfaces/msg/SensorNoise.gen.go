...
type SensorNoise struct {
	...
	Type uint8 `yaml:"type"`// The type of noise
	Mean float64 `yaml:"mean"`// Noise meanUsed by GAUSSIAN and GAUSSIAN_QUANTIZED
	Stddev float64 `yaml:"stddev"`// Noise standard deviationUsed by GAUSSIAN and GAUSSIAN_QUANTIZED
	BiasMean float64 `yaml:"bias_mean"`// Noise mean biasUsed by GAUSSIAN and GAUSSIAN_QUANTIZED
	BiasStddev float64 `yaml:"bias_stddev"`// Noise standard deviation bias
	Precision float64 `yaml:"precision"`// Noise precisionUsed by GAUSSIAN_QUANTIZED
	DynamicBiasStddev float64 `yaml:"dynamic_bias_stddev"`// For type "gaussian*", the standard deviation of the noise used todrive a process to model slow variations in a sensor bias.
	DynamicBiasCorrelationTime float64 `yaml:"dynamic_bias_correlation_time"`// For type "gaussian*", the correlation time in seconds of thenoise used to drive a process to model slow variations in a sensor bias.A typical value, when used, would be on the order of3600 seconds (1 hour).
}
