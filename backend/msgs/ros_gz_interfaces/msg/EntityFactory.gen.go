...
type EntityFactory struct {
	Name string `yaml:"name"`// New name for the entity, overrides the name on the SDF
	AllowRenaming bool `yaml:"allow_renaming"`// Whether the server is allowed to rename the entity in case of
	Sdf string `yaml:"sdf"`// SDF description in string format. Only one method is supported at a time (sdf,sdf_filename,clone_name)
	SdfFilename string `yaml:"sdf_filename"`// Full path to SDF file.
	CloneName string `yaml:"clone_name"`// Name of entity to clone
	Pose geometry_msgs_msg.Pose `yaml:"pose"`// Pose where the entity will be spawned in the world.
	RelativeTo string `yaml:"relative_to"`// Pose is defined relative to the frame of this entity.
}
