...
type TrackVisual struct {
	...
	Name string `yaml:"name"`// Name of the visual to track.
	Id uint32 `yaml:"id"`// Id of the visual to track.
	InheritOrientation bool `yaml:"inherit_orientation"`// True to have the tracking camera inherit the orientation ofthe tracked visual.
	MinDist float64 `yaml:"min_dist"`// Minimum follow distance.
	MaxDist float64 `yaml:"max_dist"`// Maximum follow distance.
	IsStatic bool `yaml:"is_static"`// If set to true, the position of the camera is fixed.
	UseModelFrame bool `yaml:"use_model_frame"`// If set to true, the position of the camera is relative to the.model reference frame.
	Xyz geometry_msgs_msg.Vector3 `yaml:"xyz"`// Position of the camera.
	InheritYaw bool `yaml:"inherit_yaw"`// If set to true, the camera inherits the yaw rotation of the model.
}
